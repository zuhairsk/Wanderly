  ...
  private seedData(): void {
    ...
    try {
      ...
      attractionsData.forEach((data) => {
        const id = randomUUID();
        const jsonAttr = jsonData.attractions.find((a: any) => a.name === data.name);
        this.attractions.set(id, {
          ...
          averageRating: jsonAttr?.rating || 4.0, // Use rating from JSON
          reviewCount: jsonAttr?.reviewCount || Math.floor(Math.random() * 1000) + 100, // Use review count from JSON
          createdAt: new Date(),
        });
      });
    ...
    if (attractionIds.length > 0) {
      ...
      // Update ratings for seeded reviews
